...
public class AsientoMapperImpl implements AsientoMapper {
    ...
    protected Venta ventaDTOToVenta(VentaDTO ventaDTO) {
        ...
        venta.asientos( asientoDTOSetToAsientoSet( ventaDTO.getAsientos() ) );
    ...
    protected void ventaDTOToVenta1(VentaDTO ventaDTO, Venta mappingTarget) {
        ...
        if ( mappingTarget.getAsientos() != null ) {
            Set<Asiento> set = asientoDTOSetToAsientoSet( ventaDTO.getAsientos() );
            if ( set != null ) {
                mappingTarget.getAsientos().clear();
                mappingTarget.getAsientos().addAll( set );
            }
        }
        else {
            Set<Asiento> set = asientoDTOSetToAsientoSet( ventaDTO.getAsientos() );
            if ( set != null ) {
                mappingTarget.asientos( set );
            }
        }
    }
}
