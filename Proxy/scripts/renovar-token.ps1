...
try {
  # Llamada a /authenticate
  $authParams = @{
    Uri         = "$serverUrl$apiBase/authenticate"
    Method      = 'Post'
    Body        = $loginBody
  ...
  if ([string]::IsNullOrWhiteSpace($newToken)) {
    throw "La respuesta no contiene id_token"
  }
  ...
  if ($envVars.JWT_TOKEN) {
    $prev = $envVars.JWT_TOKEN
    $prevShort = $prev.Substring(0, [Math]::Min(50, $prev.Length))
  ...
  for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match '^\s*JWT_TOKEN\s*=') {
      $lines[$i] = "JWT_TOKEN=$newToken"
      $found = $true
      break
    }
  }
  ...
  try {
    ...
    Write-Host "Aviso: no se pudo verificar el token en /account" -ForegroundColor Yellow
  }
  ...
  try {
    ...
    # Ignorar errores al obtener StatusCode
  }
